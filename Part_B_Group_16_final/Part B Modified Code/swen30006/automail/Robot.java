...
public class Robot {
     ...
     * @param penalty is the delivery action basically it contains how to calculate the total score
     */
    public Robot(IRobotBehaviour behaviour, IMailPool mailPool, double penalty){
        // current_state = RobotState.WAITING;
    	current_state = RobotState.RETURNING;
        current_floor = Building.MAILROOM_LOCATION;

        this.behaviour = behaviour;
        this.delivery = new ReportDelivery(penalty);
        this.mailPool = mailPool;
        this.deliveryCounter = 0;

        //set different MaxCount according to different behaviours
        if(behaviour instanceof BigSmartRobotBehaviour){
            MAX_COUNT = BigSmartRobotBehaviour.MAX_TAKE;
        }
        else if (behaviour instanceof SimpleRobotBehaviour){
            MAX_COUNT = SimpleRobotBehaviour.MAX_TAKE;
        }
        else{
            MAX_COUNT = SmartRobotBehaviour.MAX_TAKE;
        }
        //create corresponding storagetube
        tube = new StorageTube(MAX_COUNT);
    }
